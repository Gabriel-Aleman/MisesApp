...
class datos(leerDatos):
    def __init__(self, selectMode=True, filePath=f_path, stringData="", readFromDB=True):
        super().__init__(selectMode, filePath, stringData, Idle_funcs=False)
        self.connect2DB()
    ...
    def graph_IntXTime(self, df):
        ...
        return fig
    @df_dec
    def Planillas(self, df):
        df_p = self.datosDePago(df)
        fig = px.bar(df_p, x="Fechas", y="Crédito", color="Crédito", title="Planilla", color_continuous_scale="agsunset")
        return fig

    def cat_PiePlot(self,d=None):
        if d is None:
            d = self.dataFrame_Categorias_Stad()
        fig = px.pie(d, values="Cantidad", names="Categoría",
    ...
    def cat_ScatPlot(self,d=None):
        fig = px.scatter(d, x="Categoría", y="Débito", 
                        size="Débito", 
                        color="Categoría",
                        title="Dispersión de Débitos por Categoría")


        return fig
    
    def cat_BoxPlot(self,d=None):
        fig = px.box(d, x="Categoría", y="Débito", color="Categoría", title="Distribución de Débitos por Categoría")
        return fig
    #%%---------------------------------------------------------------------------------------------------------------------------
